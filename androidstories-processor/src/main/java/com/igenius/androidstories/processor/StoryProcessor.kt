...
class StoryProcessor : AbstractProcessor() {
    ...
    override fun process(set: Set<TypeElement?>?, roundEnvironment: RoundEnvironment?): Boolean {
        ...
        elements?.filterNotNull()?.forEach { element ->
            if (element.kind == ElementKind.METHOD) {
                ...
                if ((element as? TypeElement)?.superclass.toString() == "com.igenius.androidstories.app.StoryFragment") {
    ...
    private fun createFragment(element: Element): Pair<ClassName, FileSpec> {
        ...
        val file = FileSpec.builder(element.packageName, fragmentClassName)
            .addComment("Autogenerated StoryFragment to expose ${element.packageName}.${element.simpleName}")
            .addType(
                TypeSpec
                    .classBuilder(fragmentClassName)
                    .superclass(ClassName("com.igenius.androidstories.app", "StoryFragment"))
                    .addFunction(
    ...
    private fun generateFragmentStory(element: Element, fragmentName: ClassName, id: Int): String {
        ...
        val variants =
            element.getAnnotation(Story::class.java).variants
        return """
...
$description
            override val variants = arrayOf(${variants.joinToString(",") { "\"$it\"" }})
